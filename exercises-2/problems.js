window.answers = () => {
    ...
    let output = false
    if( (input > 20 && input % 2 == 0) || input < 10 && input % 2 != 0) {
        output = true
    }
    ...
    const rabbitString = numRabbits == 1 ? "There is " +  numRabbits + " rabbit" : "There are " +  numRabbits + " rabbits";
    ...
    for (i = 0; i<=1000; i++) {
        for (j = 0; j<=1000; j++)  {
            for (k = 0; k<=1000; k++) {
                if (((i**2 + j**2 == k**2)) && ((i+j+k) ==1000) && (i < j && j < k) ) {
                    a = i
                    b = j
                    c = k
                    break
